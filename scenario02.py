...
def process_scenario02_data(file):
    ...
    try:
        ...
        if has_geo_data:
            fig = _create_geo_plot(nodes_to_plot, processed_connections, data)
        else:
            fig = _create_2d_plot(nodes_to_plot, processed_connections, data)
        
        return {'graph_json': fig.to_json()}
        
    except Exception as e:
        return {'error': f"Error al procesar el archivo: {e}"}

def _create_geo_plot(nodes_to_plot, processed_connections, data):
    """Creates a geographic plot with mapbox."""
    fig = go.Figure()
...
def _create_2d_plot(nodes_to_plot, processed_connections, data):
    """Creates a 2D plot for networks without geographic data."""
    fig = go.Figure()
    node_hover_texts, node_symbols, node_colors = get_node_styles_and_tooltips(nodes_to_plot, edfa_equipment_data)
    node_coords = {el['uid']: el['metadata']['location'] for el in nodes_to_plot}
